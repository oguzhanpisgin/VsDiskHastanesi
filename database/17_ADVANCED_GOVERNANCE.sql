...
-- Version: 1.1 (fix batch and compatibility)
...
-- Safe: Idempotent
-- Notes:
--  - CREATE OR ALTER statements must be first in batch -> separated by GO
--  - Use DATABASEPROPERTYEX instead of DBPROPERTY
-- =============================================================
...
GO
-- Support tables (in a protected transaction)
BEGIN TRY
    ...
    PRINT '== ADV GOV SUPPORT TABLES READY ==';
END TRY
BEGIN CATCH
    IF @@TRANCOUNT>0 ROLLBACK TRANSACTION AdvGov;
    PRINT '? ADVANCED GOVERNANCE SUPPORT TABLES FAILED: ' + ERROR_MESSAGE();
    THROW;
END CATCH;
GO
...
END;
GO
PRINT '? sp_CaptureWaitStats';
GO

-- 3. QUERY STORE REGRESSIONS
CREATE OR ALTER PROCEDURE dbo.sp_CaptureQueryStoreRegressions @MinPlans INT=3, @MinDurationMs BIGINT=1000 AS
BEGIN
    SET NOCOUNT ON;
    IF CAST(DATABASEPROPERTYEX(DB_NAME(),'IsQueryStoreOn') AS INT)<>1 BEGIN PRINT 'Query Store not enabled'; RETURN; END
    ;WITH Q AS (
        SELECT q.query_id,
...
END;
GO
PRINT '? sp_CaptureQueryStoreRegressions';
GO
...
END;
GO
PRINT '? sp_CaptureBackupChainHealth';
GO
...
END;
GO
PRINT '? sp_SargabilityScan';
GO

-- 9. DUPLICATE INDEX RECOMMENDATIONS
...
END;
GO
PRINT '? sp_BuildDuplicateIndexRecommendations';
GO
...
END;
GO
PRINT '? sp_ReportPerformanceOverview';
GO

-- 11. CHECK DANGLING REFERENCES + VALIDATE CONSISTENCY
